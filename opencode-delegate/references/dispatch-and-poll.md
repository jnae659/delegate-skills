...
- `nestedDispatch` — `true` when the implementer attempted to re-delegate (a bash command spawning
  `relay.mjs` or another `opencode run`); `nestedCommands` lists the attempts. Treat it like a gates
  failure: do not land on it, even if a permission layer blocked the attempt from succeeding.
