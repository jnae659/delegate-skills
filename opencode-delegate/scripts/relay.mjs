...
function dispatchToOpenCode(opts, brief, run, writeResult) {
  ...
  const stderrTail = [];
  // Nested-dispatch tripwire: an implementer that tries to re-delegate (spawn the relay or a
  // second `opencode run` through bash) is a bug, not initiative. Record the attempts so the
  // review sees them even when a permission layer blocks the command from succeeding.
  const nestedCommands = [];
  const NESTED_DISPATCH = /(^|[^\w'"])node\s+[^\n]*relay\.mjs|(^|[^\w'"])opencode\s+run\b/;
  const nestedFields = () => ({ nestedDispatch: nestedCommands.length > 0, ...(nestedCommands.length ? { nestedCommands: nestedCommands.slice() } : {}) });
  ...
  const scan = makeEventScanner((event) => {
    ...
    // tool_use events carry the bash command at part.state.input.command (shape confirmed
    // against a live stream); state may be pending before completed, so read whenever present.
    if (event.type === "tool_use" && event.part && event.part.tool === "bash") {
      const command = event.part.state?.input?.command;
      if (typeof command === "string" && NESTED_DISPATCH.test(command) && !nestedCommands.includes(command)) {
  ...
  child.on("error", (err) => {
    ...
    settled = true;
    clearWatchdog();
    const result = writeResult({ status: "failed", exitCode: 1, signal: null, sessionId, finalMessage: assembleFinal(), touchedFiles: gitTouchedFiles(opts.cd), cost: sawCost ? totalCost : null, ...nestedFields(), error: String(err && err.message ? err.message : err) });
    printSummary(result, run.resultPath);
    process.exit(1);
  });
...
function printSummary(result, resultPath) {
  ...
  if (result.nestedDispatch) lines.push(`NESTED DISPATCH DETECTED: the implementer tried to re-delegate (${(result.nestedCommands || []).length} bash attempt(s), see nestedCommands in result.json) — treat as a review failure even if a permission layer blocked it.`);
