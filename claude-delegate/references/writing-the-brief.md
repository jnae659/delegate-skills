...
orchestrator to review and land. Never run php artisan migrate:fresh, migrate:refresh, or db:wipe —
forward-only migrate only; report broken schema instead of resetting.
</action_safety>

<no_nesting>
You are the implementer: do all work yourself. Never dispatch other agents, sessions, CLIs, or
relay scripts; never write briefs for others to execute; never re-delegate any part of this task.
</no_nesting>
...
<action_safety>
No unrelated refactors. Do not git add, commit, or push; leave the work uncommitted. Never run
php artisan migrate:fresh, migrate:refresh, or db:wipe — forward-only migrate only.
</action_safety>
